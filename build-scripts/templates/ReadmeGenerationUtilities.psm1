...
function Get-LibmanDependencyTable {
    ...
    foreach($lib in $libraries){
    	...
    	$tableString += "`r`n|{0}|{1}|{2}|{3}|" -f "$packageName", "$packageVersion", "$license", $purposes[$libInfo.Name]
...
function Get-LibmanLibraryData {
    ...
    return @{ Name = $name; License = $license; RepositoryUrl = $repoUrl; ProjectUrl = $url; CurrentVersion = $currentVersion; PackageBadge=$badgePackage; LicenseBadge=$badgeLicense; LicenseUrl=$licenseUrl; }
}
